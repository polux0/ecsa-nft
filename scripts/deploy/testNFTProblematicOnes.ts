...
import { TestNFTProblematicOnes } from "../../typechain";
...
async function main() {
  ...
  // Deploy the NFT
  const testNFT = await ethers.getContractFactory('TestNFTProblematicOnes', {
    libraries: {
    SVGGenerator: svgGeneratorLib.address,
    },
  });
  
  const testNFTContract = await testNFT.deploy(storageFacadeAddresses, { gasLimit: 20000000 }) as TestNFTProblematicOnes; // as TestNFT
  ...
  await run("verify:verify", {
    ...
    contract: "contracts/TestNFTProblematicOnes.sol:TestNFTProblematicOnes"
  })
}
